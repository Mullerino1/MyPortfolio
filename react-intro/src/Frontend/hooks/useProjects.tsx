...
export function useProjects(){
  ...
  const add = async (data: Partial<ProjectType>) => {
    console.log(data + '2')
    const { title = "", description = "", id= "", createdAt="", updatedAt = "", publishedAt="", status=""}  = data

    try {
      setStatus("loading")
      await projectApi.create({ title, description, id, createdAt, updatedAt, publishedAt, status})
      await fetchData()
      setStatus("success")
    } catch (error) {
      setStatus("error")
